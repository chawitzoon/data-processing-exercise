...
```mermaid
flowchart TD
    A[Phase 1: Structural Onboarding] --> B[ Schema Mapper]
...
Instead of running heavy inference across millions of daily records, an isolated  execution runs once when a new competitor source is introduced.
...
The main operational pipeline minimizes expensive  calls by applying deterministic filtering and vector retrieval before semantic verification.
...
* bypasses  verification if similarity falls below threshold

This significantly reduces unnecessary  calls for obvious `no_match` cases.

---

#### Stage 3: -Based Candidate Verification
...
*  uncertainty or execution failures
  → `uncertain`
...
> minimize the number of rows that require expensive  verification.
...
while only a small ambiguous subset is escalated to  semantic verification.
...
The  profiles the incoming competitor source and generates:
...
This would significantly improve retrieval precision before  verification.
...
The decision confidence scores are should be based on the similarity search score. 
Those in initial submission are generated by the LLM but in practice, it is prefer to get the score from non-LLM instead.
